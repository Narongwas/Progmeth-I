...
public class GameSystem {
	...
	public BaseUnit promote(BaseUnit baseUnit,int choice) {
		BaseUnit unit = null;
		if(choice == 0) {
			unit = new MeleeUnit(baseUnit.getColumn(),baseUnit.getRow(),baseUnit.isWhite(),baseUnit.getName());
		} else if (choice == 1) {
			unit = new RangeUnit(baseUnit.getColumn(),baseUnit.getRow(),baseUnit.isWhite(),baseUnit.getName());
		} else if (choice == 2) {
			unit = new FlyingUnit(baseUnit.getColumn(),baseUnit.getRow(),baseUnit.isWhite(),baseUnit.getName());
		}
		return unit;
	}
